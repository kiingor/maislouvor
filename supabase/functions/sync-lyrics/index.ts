...
const INSTRUMENTAL_TAGS =
  /^\[(intro|solo|instrumental|interlude|interlúdio|outro|prelúdio|pré-refrão|pos-refrão|ponte)\]$/i;
...
function isInstrumentalSegment(segment: string): boolean {
  ...
  const nonTagLines = lines.filter((l) =>
    !INSTRUMENTAL_TAGS.test(l) && !/^\[.*\]$/.test(l)
  );
...
function countSingableWords(segment: string): number {
  const chordPattern =
    /^[A-G][#b]?(?:m|maj|min|dim|aug|sus[24]?|add\d+|7|9|11|13|6)*(?:\/[A-G][#b]?)?$/;
  const sectionTag = /^\[.*\]$/;
  let count = 0;
...
function generateFallbackTimestamps(
  segments: string[],
  duration: number,
): number[] {
...
function normalizeTimestamps(
  ...
  duration: number,
  segments: string[],
): { timestamps: number[]; adjusted: boolean } {
  let adjusted = false;
  ...
  if (ts.length < segmentCount || ts.some((v) => v < 0)) {
    return {
      timestamps: generateFallbackTimestamps(segments, duration),
      adjusted: true,
    };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const body = await readJsonBody(req, 150_000);
    const song = await requireSongEditor(context, body.song_id);
    await enforceRateLimit(context, "sync-lyrics", song.team_id, 20, 600);
    ...
    if (
      ...
      segments.some((segment) =>
        typeof segment !== "string" || segment.length > 20_000
      )
    ) {
      throw new HttpError(400, "segments inválido");
    }
    if (
      ...
      throw new HttpError(400, "duration_seconds inválido");
    }

    const LOVABLE_API_KEY = requiredEnv("LOVABLE_API_KEY");

    const systemPrompt =
      `You are a music timing expert. Given lyric/chord segments and total song duration, estimate when each segment starts.
...
${
      segments.map((s: string, i: number) => {
        ...
        const label = instrumental
          ? "[INSTRUMENTAL]"
          : `(${words} singable words)`;
...
Return ONLY the array of ${segments.length} timestamps using the suggest_timestamps tool.`;
    ...
    const response = await fetch(
      "https://ai.gateway.lovable.dev/v1/chat/completions",
      {
        ...
        body: JSON.stringify({
          ...
          tools: [
            {
              ...
              function: {
                ...
                description:
                  "Return an array of timestamps (in seconds) for when each segment starts in the song.",
                parameters: {
                  type: "object",
                  properties: {
                    timestamps: {
                      ...
                      items: { type: "number" },
                      description:
                        "Array of start times in seconds for each segment",
                    },
                  },
                  required: ["timestamps"],
          ...
          tool_choice: {
            type: "function",
            function: { name: "suggest_timestamps" },
          },
        }),
      },
    );
    ...
    const { timestamps, adjusted } = normalizeTimestamps(
      rawTimestamps,
      segments.length,
      duration_seconds,
      segments,
    ...
    return json({ timestamps, adjusted });
  } catch (e) {
